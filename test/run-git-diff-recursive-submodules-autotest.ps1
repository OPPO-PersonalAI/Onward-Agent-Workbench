...
$fixture = node (Join-Path $RootDir "test\create-recursive-git-submodule-fixture.mjs") | ConvertFrom-Json
$FixtureTempRoot = $fixture.tempRoot
$TargetRepo = $fixture.repoRoot
...
try {
  ...
  Write-Host "Starting Git Diff recursive-submodule autotest..."
  ...
  $env:ONWARD_AUTOTEST_SUITE = "git-diff-recursive-submodules"
  ...
  if ($content -match "\[AutoTest\] FAIL") {
    Write-Host "Git Diff recursive-submodule autotest FAILED" -ForegroundColor Red
    Write-Host ""
  ...
  if ($content -notmatch "RSM-01-root-only-discovers-nested-submodules") {
    Write-Host "Missing RSM-01 result; the test may not have executed correctly" -ForegroundColor Yellow
    Get-Content $LogFile -Tail 40
    exit 1
  }

  Write-Host "Git Diff recursive-submodule autotest PASSED" -ForegroundColor Green
  Write-Host "  Log: $LogFile"
} finally {
  if ($FixtureTempRoot -and (Test-Path $FixtureTempRoot)) {
    Remove-Item -Recurse -Force $FixtureTempRoot
  }
}
