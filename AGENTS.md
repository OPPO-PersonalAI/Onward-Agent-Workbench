...
- Hard rule — Launching the app for testing: before opening the built `.app`, you must first kill any existing instance of the same application using **exact name matching** (`pkill -x "<exact-process-name>"`). Never use wildcards or partial matches. Chain the kill and open in one command: `pkill -x "Onward 2-<branch>" 2>/dev/null; sleep 0.5; open "<path-to-.app>"`.
    - **Absolutely forbidden during testing**: broad patterns such as `pkill -9 "Onward"`, `pkill "Onward 2"`, `pkill -f onward`, `pkill "Onward 2 Helper"`, or any `pkill` invocation that resolves via substring/regex instead of the exact process name. Such commands also terminate the user's Claude Code session, other Onward branch builds, unrelated helpers, and anything whose name merely contains "Onward". This applies even to the `-9` signal: SIGKILL does not excuse a loose selector.
    - The exact process name MUST be passed as the final argument with `-x` and nothing else. Helper processes (for example, `Onward 2 Helper`, `Onward 2 Helper (GPU)`, `Onward 2 Helper (Renderer)`) are cleaned up automatically by the OS once the parent main process exits — do NOT kill them yourself.
